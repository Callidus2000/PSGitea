...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'Gitea.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'Gitea.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'Gitea.Client.ProtectedToken'

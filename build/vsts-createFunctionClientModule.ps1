...
$parentModule = 'Gitea'
if (-not $ModuleName) { $ModuleName = 'Gitea.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\Gitea'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\Gitea.psd1"
$prereqHash = @{
